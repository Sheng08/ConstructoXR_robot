...
def handleSerialData(raw_data):
    ...
    if key < buff[2] + 5:  # 根据数据长度位的判断, 来获取对应长度数据
        ...
        if buff[2] == 0x2c and pub_flag[0]:
            ...
            print("該數據處理類沒有提供該 " + str(buff[2]) + " 的解析")
            print("或數據錯誤")
            buff = {}
            key = 0
...
if __name__ == "__main__":
    ...
    try:
        ...
        if hf_imu.isOpen():
            rospy.loginfo("\033[32m串口打開成功...\033[0m")
        else:
            hf_imu.open()
            rospy.loginfo("\033[32m打開串口成功...\033[0m")
    except Exception as e:
        print(e)
        rospy.loginfo("\033[31m串口打開失敗\033[0m")
        exit(0)
    else:
